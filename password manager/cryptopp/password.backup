...
std::string Decrypt(const CryptoPP::byte* ciphertext, size_t length,
        ...
        ArraySource(ciphertext, length, true,
            new StreamTransformationFilter(decryptor,
...
public:
    ...
    CryptoPP::byte password_byte[1024];
    string password_byte_str,iv_byte_str;
    ...
    Password(string website,string username,string password, string iv):website(website),username(username),password(password),iv(iv){
        ...
        //将password转换为byte数组,并记录长度
        HexDecoder decoder2;
        decoder2.Attach(new ArraySink(password_byte, 1024));
        decoder2.Put((const CryptoPP::byte*)password.data(), password.size());
        decoder2.MessageEnd();
        password_length = decoder2.MaxRetrievable();

        // 1. 将 password_byte[] 转成 hex 字符串
        CryptoPP::StringSource(password_byte, sizeof(password_byte), true,
            new CryptoPP::HexEncoder(
                new CryptoPP::StringSink(password_byte_str),
                false // 不换行
            )
        );

        // 2. 将 iv_byte[] 转成 hex 字符串
        CryptoPP::StringSource(iv_byte, sizeof(iv_byte), true,
            new CryptoPP::HexEncoder(
                new CryptoPP::StringSink(iv_byte_str),
                false // 不换行
            )
        );
    }
    Password(string password_line){
        ...
        getline(ss,website,',');
        getline(ss,username,',');
        getline(ss,password_byte_str,',');
        getline(ss,iv_byte_str,',');
        std::copy(password_byte_str.begin(), password_byte_str.end(), password_byte);
        std::copy(iv_byte_str.begin(), iv_byte_str.end(), iv_byte);
        password_length = password_byte_str.length() / 2;
        }
    string csv_format() const{
        std::stringstream ss;
        ss << website << "," << username << ",";
        ss << password_byte_str << "," << iv_byte_str << "\n";
        return ss.str();
    }
...
public: 
    ...
    void show(int index) {   //显示指定index的Password对象的show()函数返回值
        ...
        AES_Processer aes_processor;
        string decrypted_password = aes_processor.Decrypt(_passwords[index].password_byte_str, key_byte, _passwords[index].iv_byte);
            ...
            switch(choice) {
                ...
                case 2:
                    ...
                    if(website_input.compare("#")==0){   //进入索引模式
                        ...
                        index_input = serach(website_input);
                    ...
                    cout << "网址：" << _passwords[index_input].website << "  " << "用户名：" << _passwords[index_input].username << endl;
                    cout << "请选择操作：" << endl;
                    cout << "1. 查看密码" << endl;
                    cout << "2. 编辑密码" << endl;
                    cout << "3. 删除密码" << endl;
                    ...
                    if (choice2 == 4) {
                        ...
                        switch(choice2) {
                            case 1:
                                show(index_input);   //显示指定index的Password对象的show()函数返回值
                            
                                pause();
                                break;
                            case 2:
                                edit(index_input);   //编辑指定index的Password对象的edit()函数返回值
                                
                                pause();
                                break;
                            case 3:
                                del(index_input);   //删除指定index的Password对象的del()函数返回值
